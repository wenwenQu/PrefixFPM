...
//extern TimeTracker t1,t2,t3,t4;
//extern int cnt;
char infile[300];
...
void parse_args(int argc, char **argv) {
		...
		if (argc < 2)
			...
			while ((c = getopt(argc, argv, "e:i:s:n:c:T:t:b:B")) != -1) {
				switch (c) {
				case 'i': //input files
				...
				case 't': // projDB size threshold, below which task is run in a single thread
					...
					break;
				case 'e': // task running time threshold, below which task is inserted into queue directly
					tauDB_time = atof(optarg);
					break;
