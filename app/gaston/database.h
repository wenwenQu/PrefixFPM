...
typedef int InputEdgeLabel;
typedef int InputNodeLabel;
typedef int InputNodeId;
typedef unsigned long CombinedInputLabel;
#define combineInputLabels(label1,label2,label3) (label1 | ( ((unsigned long) label2 ) << 28 ) | ( ( (unsigned long) label3 ) << 56 ) ) // by wenwen : use bit to represent edge
...
struct DatabaseTree {
  Tid tid;
  vector<DatabaseTreeNode> nodes; // by wenwen : nodes[pos] = label
...
struct DatabaseNodeLabel {
  ...
  Frequency frequency;
  Tid lasttid; //by wenwen : the last transaction id contain the label
...
class Database {
  public:
    ...
    vector<DatabaseNodeLabel> nodelabels; // by wenwen: nodelabels[nodelabel] = inputnodelabel
    vector<DatabaseEdgeLabel> edgelabels;
    map<InputNodeLabel,NodeLabel> nodelabelmap; //by wenwen: relabel the node
    ...
    void reorder () {
      ...
      //by wenwen : clear infrequent node and label
      for ( Tid i = 0; i < trees.size (); i++ ) {
        ...
        for ( NodeId j = 0; j < tree.nodes.size (); j++ ) {
          ...
          if ( nodelabels[node.nodelabel].frequency >= minfreq ) {
            ...
            nodelabel.occurrences.elements.emplace_back (tree.tid, (OccurrenceId) nodelabel.occurrences.elements.size (),j,NONODE);
            int k = 0;
  ...
  private:
    void readTree ( FILE *input, Tid tid ) {
      ...
      while ( !feof ( input ) && command == 'e' ) {
        ...
        if(nodeid1 >= nodessize || nodeid2 >= nodessize){
        	command = readcommand ( input );
        	continue;
        }
